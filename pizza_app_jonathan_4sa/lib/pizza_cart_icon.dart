...
class _PizzaCartIconState extends State<PizzaCartIcon> with SingleTickerProviderStateMixin {
  ...
  @override
  void dispose() {
    _controller.dispose(); // Libera los recursos del controlador de animación cuando el widget se destruye.
    super.dispose();
  }
    ...
    return AnimatedBuilder(
      ...
      builder: (context, snapshot) {
        ...
        if (_animationScaleOut.value < 1.0) {
          scale = 1.0 + scaleFactor * _animationScaleOut.value; // Expande el ícono.
        } else if (_animationScaleIn.value <= 1.0) {
          scale = (1.0 + scaleFactor) - scaleFactor * _animationScaleIn.value; // Contrae el ícono.
        }
              ...
              if (_animationScaleOut.value > 0)
                Positioned(
                  ...
                  child: Transform.scale(
                    alignment: Alignment.center,
                    scale: _animationScaleOut.value, // Escala la burbuja del contador.
                    child: CircleAvatar(
                      radius: 7,
                      backgroundColor: Colors.red, // Fondo rojo para resaltar el contador.
                      child: Text(
                        counter.toString(), // Muestra el número de elementos en el carrito.
