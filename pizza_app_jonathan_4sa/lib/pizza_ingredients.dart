    ...
    return ValueListenableBuilder(
      ...
      builder: (context, value, _) {
        ...
        return ListView.builder(
          scrollDirection: Axis.horizontal, // Configura la dirección del scroll a horizontal.
          itemCount: ingredients.length, // Cantidad total de ingredientes en la lista.
          itemBuilder: (context, index) {
            final ingredient = ingredients[index];
            return PizzaIngredientItem(
              ingredient: ingredient, // Pasar el ingrediente a cada ítem.
              exist: bloc.containsIngredient(ingredient), // Verifica si el ingrediente está en la pizza.
              onTap: () {
                bloc.removeIngredient(ingredient); // Elimina el ingrediente de la pizza al hacer tap.
              },
            );
          },
        );
...
class PizzaIngredientItem extends StatelessWidget {
  ...
  Widget _buildChild({bool withImage = true}) {
    return GestureDetector(
      onTap: exist ? onTap : null, // Si el ingrediente existe, se puede eliminar al hacer tap.
      child: Padding(
        padding: const EdgeInsets.symmetric(horizontal: 7.0),
        child: Container(
          height: itemSize, // Establece el tamaño del contenedor para el ingrediente.
          width: itemSize, 
          decoration: BoxDecoration(
            color: const Color(0xFFF5EED3), // Color de fondo del ingrediente.
            shape: BoxShape.circle, // Forma circular.
            border: exist ? Border.all(color: Colors.red, width: 2) : null, // Si el ingrediente existe, bordea con rojo.
          ),
          child: Padding(
            padding: const EdgeInsets.all(5.0),
            child: withImage
                ? Image.asset( // Si `withImage` es verdadero, muestra la imagen del ingrediente.
                    ingredient.image,
                    fit: BoxFit.contain, // Ajusta la imagen al contenedor sin distorsión.
                  )
                : SizedBox.fromSize(), // Si `withImage` es falso, no muestra nada.
          ),
        ),
      ),
    );
      ...
      child: exist // Si el ingrediente está en la pizza, solo lo muestra.
          ...
          : Draggable(
              ...
              feedback: DecoratedBox(
                ...
                child: _buildChild(), // Muestra el mismo contenido del ingrediente al arrastrar.
              ),
              childWhenDragging: _buildChild(withImage: false), // Muestra solo el contorno cuando se está arrastrando.
