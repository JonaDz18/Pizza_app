...
import 'package:pizza_app_jonathan_4sa/pizza_cart_button.dart';
import 'package:pizza_app_jonathan_4sa/pizza_cart_icon.dart';
import 'package:pizza_app_jonathan_4sa/pizza_ingredients.dart';
import 'package:pizza_app_jonathan_4sa/pizza_order_bloc.dart';
import 'package:pizza_app_jonathan_4sa/pizza_order_provider.dart';
import 'package:pizza_app_jonathan_4sa/pizza_size_button.dart';
        ...
        body: Stack(
          children: [
            ...
            Positioned(
              ...
              left: MediaQuery.of(context).size.width / 2 - _pizzaCartSize / 2, // Centra el botón.
...
class _PizzaDetailsState extends State<_PizzaDetails>
  ...
  Widget _buildIngredientsWidget(Ingredient? deletedIngredient) {
    ...
    return SizedBox.fromSize(); // Retorna un widget vacío si no hay animaciones.
  }
      ...
      Expanded(
        ...
        child: DragTarget<Ingredient>(
          ...
          onWillAccept: (ingredient) {
            ...
            return !bloc.containsIngredient(ingredient!); // Acepta si no está ya en la pizza.
          },
          onLeave: (ingredient) {
            print('onLeave');
            bloc.notifierFocused.value = false; // Marca como no enfocado.
          ...
          builder: (context, list, rejects) {
            return LayoutBuilder(builder: (context, constraints) {
              _pizzaConstraints = constraints; // Guarda las restricciones del área.
              ...
              return ValueListenableBuilder<PizzaMetadata?>(
                ...
                builder: (context, data, child) {
                  ...
                  return AnimatedOpacity(
                    duration: const Duration(milliseconds: 60), // Animación de opacidad.
                    opacity: data != null ? 0.0 : 1, // Oculta cuando hay datos.
                    child: ValueListenableBuilder<PizzaSizeState>(
                      valueListenable: bloc.notifierPizzaSize,
                      builder: (context, pizzaSize, _) {
                        return RepaintBoundary(
                          key: _keyPizza, // Llave para capturar el área de renderizado.
                          child: RotationTransition(
                            turns: CurvedAnimation(
                              curve: Curves.elasticInOut, // Curva de rotación elástica.
                              ...
                              children: [
                                        ...
                                        child: Stack(
                                          children: [
                                            DecoratedBox(
                                              ...
                                              child: Image.asset('lib/images/dish.png'), // Plato base.
                                            ),
                                            Padding(
                                              padding: const EdgeInsets.all(10.0),
                                              child: Image.asset('lib/images/pizza-1.png'), // Pizza base.
                                            ),
                                          ],
                                        ),
                                      );
                                ...
                                ValueListenableBuilder<Ingredient?>(
                                  ...
                                  builder: (context, deletedIngredient, _) {
                                    _animateDeletedIngredient(deletedIngredient); // Maneja animación al eliminar.

                                    return AnimatedBuilder(
                                      animation: _animationController,
                                      builder: (context, _) {
                                        return _buildIngredientsWidget(deletedIngredient); // Construye los ingredientes.
                                      },
                                    );
                                  },
                                ),
...
void _addPizzaToCart() {
  ...
  RenderRepaintBoundary? boundary = _keyPizza.currentContext!.findRenderObject() as RenderRepaintBoundary?;
...
class _PizzaOrderAnimationState extends State<PizzaOrderAnimation>
  ...
  late Animation<double> _pizzaScaleAnimation; // Escala de la pizza al reducirse.
  late Animation<double> _pizzaOpacityAnimation; // Desvanecimiento de la pizza.
  late Animation<double> _boxEnterScaleAnimation; // Escala inicial al entrar la caja.
  late Animation<double> _boxExitScaleAnimation; // Escala al salir la caja.
  late Animation<double> _boxExitToCartAnimation; // Movimiento de la caja hacia el carrito.
  ...
  void initState() {
    ...
    _controller = AnimationController(
      vsync: this,
      duration: const Duration(milliseconds: 2500), // Duración total de la animación.
    );

    // Configuración de las animaciones específicas.
    _pizzaScaleAnimation = Tween(begin: 1.0, end: 0.5).animate(
      CurvedAnimation(
        curve: const Interval(0.0, 0.2), // Ocurre en los primeros 20% del tiempo.
        parent: _controller,
      ),
    );
    ...
    _boxEnterScaleAnimation = CurvedAnimation(
      curve: const Interval(0.0, 0.2), // Entrada de la caja en los primeros 20%.
      parent: _controller,
    );

    _boxExitScaleAnimation = Tween(begin: 1.0, end: 1.3).animate(
      CurvedAnimation(
        curve: const Interval(0.5, 0.7), // Escala de salida entre el 50% y el 70%.
        parent: _controller,
      ),
    );

    _boxExitToCartAnimation = CurvedAnimation(
      curve: const Interval(0.8, 1.0), // Movimiento al carrito entre el 80% y el 100%.
      parent: _controller,
    );
      ...
      child: GestureDetector(
        ...
        child: AnimatedBuilder(
          ...
          builder: (context, snapshot) {
            ...
            return Opacity(
              opacity: 1 - _boxExitToCartAnimation.value, // Gradualmente desaparece.
              child: Transform(
                alignment: Alignment.center,
                transform: Matrix4.identity()
                  ..translate(moveToX, moveToY) // Mueve la caja hacia el carrito.
                  ..rotateZ(_boxExitToCartAnimation.value) // Rotación opcional.
                  ..scale(_boxExitScaleAnimation.value), // Cambia el tamaño de la caja.
                    ...
                    children: [
                      ...
                      Opacity(
                        ...
                        child: Transform(
                          alignment: Alignment.center,
                          transform: Matrix4.identity()
                            ..scale(_pizzaScaleAnimation.value) // Cambia el tamaño de la pizza.
                            ..translate(
                              0.0,
                              20 * (1 - _pizzaOpacityAnimation.value), // Desplazamiento vertical.
                            ),
                          child: Image.memory(widget.metadata.imageBytes), // Imagen de la pizza.
                        ),
                      ),
                    ],
                  ),
...
Widget _buildBox() {
  return LayoutBuilder(
    builder: (context, constraints) {
      ...
      final boxHeight = constraints.maxHeight / 2.0; // Altura de la caja (mitad del contenedor).
      final boxWidth = constraints.maxWidth / 2.0; // Ancho de la caja (mitad del contenedor).
      ...
      return Opacity(
        opacity: _boxEnterScaleAnimation.value, // Controla la visibilidad inicial de la caja.
        child: Transform.scale(
          scale: _boxEnterScaleAnimation.value, // Escala inicial de la caja al entrar.
          child: Stack(
            children: [
              // Parte interna inferior de la caja.
              Center(
                child: Transform(
                  alignment: Alignment.topCenter, // Ajusta el punto de rotación al centro superior.
                  transform: Matrix4.identity()
                    ..setEntry(3, 2, 0.003) // Perspectiva para la rotación.
                    ..rotateX(
                      degreesToRads(minAngle), // Rota la parte inferior un ángulo fijo.
                    ),
                  child: Image.asset(
              ...
              Center(
                child: Transform(
                  ...
                  transform: Matrix4.identity()
                    ..setEntry(3, 2, 0.003) // Perspectiva para la rotación.
                    ..rotateX(
                      degreesToRads(boxClosingValue!), // Rota la parte superior según el valor interpolado.
                    ),
                  child: Image.asset(
                ...
                Center(
                  child: Transform(
                    ...
                    transform: Matrix4.identity()
                      ..setEntry(3, 2, 0.003) // Perspectiva para la rotación.
                      ..rotateX(
                        degreesToRads(boxClosingValue), // Rota la parte frontal según el valor interpolado.
                      ),
                    child: Image.asset(
