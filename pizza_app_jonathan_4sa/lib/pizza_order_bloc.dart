...
import 'package:pizza_app_jonathan_4sa/ingredient.dart'; 
...
class PizzaSizeState {
  PizzaSizeState(this.value) : factor = _getFactorBySize(value); // Determina el factor de escala según el tamaño.
  final PizzaSizeValue value; // El valor del tamaño de la pizza.
  final double factor; // Factor de escala que se utiliza para ajustar el tamaño de la pizza.
...
class PizzaOrderBloc extends ChangeNotifier {
  ...
  final notifierTotal = ValueNotifier(initialTotal); // Notificador para el total de ingredientes.
  final notifierDeletedIngredient = ValueNotifier<Ingredient?>(null); // Notificador para un ingrediente eliminado.
  final notifierFocused = ValueNotifier(false); // Notificador para si la pizza está enfocada.
  final notifierPizzaSize = ValueNotifier<PizzaSizeState>(PizzaSizeState(PizzaSizeValue.m)); // Notificador para el tamaño de la pizza.
  final notifierPizzaBoxAnimation = ValueNotifier(false); // Notificador para la animación de la caja de pizza.
  final notifierImagePizza = ValueNotifier<PizzaMetadata?>(null); // Notificador para la imagen de la pizza.
  final notifierCartIconAnimation = ValueNotifier(0); // Notificador para la animación del ícono del carrito de compras.
  ...
  void removeIngredient (Ingredient ingredient) {
    ...
    notifierDeletedIngredient.value = ingredient; // Notifica qué ingrediente fue eliminado.
  }

  // Método para restablecer el ingrediente eliminado (si es necesario).
  void refreshDeletedIngredient() {
    notifierDeletedIngredient.value = null; // Reinicia el ingrediente eliminado.
  }
  ...
  bool containsIngredient(Ingredient ingredient) {
    for (Ingredient i in listIngredients) {
      if (i.compare(ingredient)) { // Compara los ingredientes.
  ...
  Future<void> transformToImage(RenderRepaintBoundary boundary) async {
    final position = boundary.localToGlobal(Offset.zero); // Obtiene la posición de la pizza.
    final size = boundary.size; // Obtiene el tamaño de la pizza.
    final image = await boundary.toImage(); // Convierte el boundary a una imagen.
    ByteData? byteData = await image.toByteData(format: ImageByteFormat.png); // Convierte la imagen a formato PNG.
    notifierImagePizza.value = PizzaMetadata(byteData!.buffer.asUint8List(), position, size); // Notifica los metadatos de la imagen.
  }
}